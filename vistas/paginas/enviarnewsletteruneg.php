...
if(isset($_SESSION["validarIngreso"])){
  if($_SESSION["validarIngreso"]!="ok"){
    echo '<script>window.location ="index.php?pag=enviarnewsletteruneg";</script>';
    return;
  }
}else{
...
if(isset($_GET["user"])){
  $item="id_unegocio";
  $valor=$_GET["user"];
  $usuarios=ControladorFormularios::ctrSelecionarServiciosxCliente($item, $valor); 
}
?>
...
if (isset($_POST['enviar'])) {
  if(!empty($_POST['asunto']) && !empty($_POST['mytextarea'])){
    ...
    foreach ($usuarios as $key => $value):
      ...
      $oMail->addAddress($cliente["mail"],$cliente['nombre']);
